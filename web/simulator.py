...
if __name__ == "__main__":
    ...
    for index in range(m_lines):
        if index < csv_map_data.shape[0]:
            requests.post("http://0.0.0.0:8050/map_position_insert", data=json.dumps(csv_map_data.iloc[index].to_dict()))
        if index < csv_decomposition.shape[0]:
            requests.post("http://0.0.0.0:8050/variable_decomposition_insert", data=json.dumps(csv_decomposition.iloc[index].to_dict()))

...
def mqtt_sender(map_data, decomposition_data, timeout):
    ...
    for index in range(m_lines):
        ...
        if index < decomposition_data.shape[0]:
            client.publish(topic_decomposition+"/robot_1", json.dumps(decomposition_data.iloc[index].to_dict()))
            client.publish(topic_decomposition+"/robot_2", json.dumps(decomposition_data.iloc[index].to_dict()))
        # Wait timeout seconds
        time.sleep(0.1)
